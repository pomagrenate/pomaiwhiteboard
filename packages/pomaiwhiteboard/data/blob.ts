...
import type { AppState, DataURL, LibraryItem, BinaryFiles } from "../types";
...
export const getMimeType = (blob: Blob | string): string => {
  ...
  if (/\.(pomaiwhiteboard|excalidraw|json)$/.test(name)) {
  ...
  } else if (/\.(pomailib|pomaiwhiteboardlib|excalidrawlib)$/.test(name)) {
    return MIME_TYPES.pomailib || MIME_TYPES.pomaiwhiteboardlib;
  }
  return "";
};
...
export const getFileHandleType = (handle: FileSystemFileHandle | null) => {
  ...
  return handle.name.match(/\.(json|pomaiwhiteboard|excalidraw|png|svg)$/)?.[1] || null;
};
...
export const loadSceneOrLibraryFromBlob = async (
  ...
  | {
      type: typeof MIME_TYPES.pomaiwhiteboard | typeof MIME_TYPES.excalidraw;
      data: {
  ...
  | {
      type:
        | typeof MIME_TYPES.pomaiwhiteboardlib
        | typeof MIME_TYPES.excalidrawlib
        | typeof MIME_TYPES.pomailib;
...
export const loadFromBlob = async (
  ...
  if (ret.type !== MIME_TYPES.pomaiwhiteboard) {
    throw new Error("Error: invalid file");
  }
  return ret.data;
};
...
export const normalizeFile = async (file: File) => {
  ...
  if (
    file?.name?.endsWith(".pomailib") ||
    file?.name?.endsWith(".pomaiwhiteboardlib") ||
    file?.name?.endsWith(".excalidrawlib")
  ) {
    file = createFile(file, MIME_TYPES.pomailib || MIME_TYPES.pomaiwhiteboardlib, file.name);
  } else if (file?.name?.endsWith(".pomaiwhiteboard") || file?.name?.endsWith(".excalidraw")) {
    file = createFile(file, MIME_TYPES.pomaiwhiteboard, file.name);
